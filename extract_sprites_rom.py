...
PROM_CACHE = "/tmp/kof95_prom.bin"
OUTPUT_DIR = "rom_sprites_final"

NAMES = [
    "Heidern", "Ralf Jones", "Clark Still",                   # 0-2   Ikari Warriors
    "Athena Asamiya", "Sie Kensou", "Chin Gentsai",           # 3-5   Psycho Soldier
    "Kyo Kusanagi", "Benimaru Nikaido", "Goro Daimon",        # 6-8   Japan
    "Iori Yagami", "Eiji Kisaragi", "Billy Kane",             # 9-11  Rival
    "Kim Kaphwan", "Chang Koehan", "Choi Bounge",             # 12-14 Kim Team
    "Terry Bogard", "Andy Bogard", "Joe Higashi",             # 15-17 Fatal Fury
    "Ryo Sakazaki", "Robert Garcia", "Takuma Sakazaki",       # 18-20 Art of Fighting
    "Yuri Sakazaki", "Mai Shiranui", "King",                  # 21-23 Women
    "Saisyu Kusanagi", "Rugal Bernstein",                     # 24-25 Bosses
    "Omega Rugal", "Nakoruru", "Edit Team 1", "Edit Team 2",  # 26-29
]
...
C_ROM_LAYOUT = [
    ("084-c1.c1", "084-c2.c2", 0x000000),
    ("084-c3.c3", "084-c4.c4", 0x800000),
    ("084-c5.c5", "084-c6.c6", 0x1000000),
    ("084-c7.c7", "084-c8.c8", 0x1800000),
]
TOTAL_SPRITE_REGION = 0x1A00000
...
def read_sprite_def(prom, sdef_table_addr, idx):
    ...
    # Read bitmasks: BYTE (8 bits) if tiles_per_col <= 8, WORD (16 bits) if > 8
    # This matches the 68k renderer at $44F4: cmpi.w #8 → bls $4584 (byte path)
    bitmasks = []
    if tiles_per_col <= 8:
        # Byte bitmask: 1 byte per column
        for c in range(cols):
        ...
        # Word bitmask: 2 bytes per column (big-endian, 16 bits)
        for c in range(cols):
...
def render_sdef(spr_data, sdef, palette):
    ...
    for col in range(cols):
        bm = bitmasks[col] if col < len(bitmasks) else (0xFFFF if tpc > 8 else 0xFF)
        bm_bits = 16 if tpc > 8 else 8
